...
public function save($ids = "")
	{
		...
		$allowed_packages_id = post('reseller_allowed_package');
		$allowed_users = post('reseller_allowed_users');
		...
		if(!$item){
		    ...
		    if(_u('id')){
				$uid = _u('id');
				$current_user_role = _u("role");
					if($current_user_role==2){

						//$users = $this->model->get("*", , "");
						$query = $this->db->query("SELECT * FROM sp_users where created_by = '{$uid}'");
						$records = $query->num_rows();
						$reseller_allowed_user = _u("reseller_allowed_users");

						if($reseller_allowed_user <= $records){

							ms([
							"status" => "error",
							"message" => __('your number of allowed users are '.$reseller_allowed_user)
						]);
			...
			//validate('number', __('Phone Number'), $phone);
			...
			$id = $this->model->insert($this->tb_users , [
				...
				"package" => $package_id,
				"reseller_allowed_package" => json_encode($allowed_packages_id),
				"reseller_allowed_users" => $allowed_users,
