...
class upload_files extends MY_Controller {
	...
	public function index()
	{
		...
		if(_p("file_manager_video") && !_p("file_manager_photo")){
			$support_types = 'mp4|mp3|ogg|pdf|txt|pptx|ppt|docx|doc|rtf|xlsx|xls|csv';
		}

		if(_p("file_manager_photo") && _p("file_manager_video")){
			$support_types = 'gif|jpg|jpeg|png|mp4|mp3|ogg|pdf|txt|pptx|ppt|docx|doc|rtf|xlsx|xls|csv';
		}
        ...
        if ( !empty( $_FILES ) )
        {
		    ...
		    {  
		        ...
		        if( !is_type('photo', $file_type) && !is_type('video', $file_type) && !is_type('audio', $file_type) && !is_type('document', $file_type) ){
					ms([
						'status' => 'error',
				...
				if( !_p('file_manager_video') && is_type('audio', $file_type) ){
					ms([
						'status' => 'error',
						'message' => __('You do not have permission to upload videos')
					]);
