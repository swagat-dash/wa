...
class DoniaWeB{
	...
	public function __construct(){ 
		$this->product_id = 'AD4B6CBF';
		$this->api_url = 'https://api.doniaweb.com/';
		$this->api_key = '6BBAB6E72ED32D6FCA20';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["2e042ac069375a6"])){
				$_SESSION["2e042ac069375a6"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["2e042ac069375a6"])){
				$get_data = $this->call_api(
					'POST',
